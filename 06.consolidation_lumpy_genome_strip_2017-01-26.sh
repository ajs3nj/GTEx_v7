...
# For high confidence, GSCNQUAL for GS duplications >= 9, deletions >=1, and mixed >=8
...
zcat $LUMPY $GS \
    ...
    | vawk --header '{split(I$STRANDS,x,","); split(x[1],y,":"); split(x[2],z,":"); \
        if($3~"^GS_DEL" && I$GSCNQUAL>=1){ $7="PASS"; print} \
        else if($3~"^GS_DUP" && I$GSCNQUAL>=9){ $7="PASS"; print} \
        else if($3~"^GS_MIXED" && I$GSCNQUAL>=8){ $7="PASS"; print} \
...
zcat gtex.lumpy.gs.vcf.gz | /gscmnt/gc2719/halllab/users/cchiang/projects/gtex/src/vcfToBed.py | awk '$4~/^GS/ || $1~"^#"' | awk '$8=="PASS" || $8=="LOW"' |  bgzip -c > 1KGP_comparison/gtex.gs.bed.gz

zcat 1KGP_comparison/gtex.gs.bed.gz \
  | vawk -c 9 '{print $1,$2,$3,$4,I$GSCNCATEGORY}' \
  | bedtools intersect -f 0.25 -r -wo -a stdin -b /gscmnt/gc2719/halllab/users/cchiang/projects/gtex/1kg_sv_truthset_2015-12-15/ALL.wgs.integrated_sv_map_v2.20130502.svs.genotypes.bed.gz \
...
zcat gtex.lumpy.gs.1KGP_0.5.vcf.gz \
    | awk '$1 !~ /^GL/' \
    | vawk --header '$1 != "Y"' \
    | /gscmnt/gc2719/halllab/users/cchiang/projects/gtex/src/vcf_allele_freq_gs.py \
    | bgzip -c \
...
do
    ...
    tabix -h $VCF X Y | vawk '(I$SVTYPE=="DEL" || I$SVTYPE=="DUP") && I$NSAMP<=10' | vcfToBed | bedtools intersect -a stdin -b /gscmnt/gc2719/halllab/users/cchiang/projects/gtex/annotations/par.bed.gz \
        | cut -f 4 \
...
# filter gs calls by gscnqual
zcat gtex.lumpy.gs.1KGP_0.5.size.high_conf.vcf.gz \
    | vawk --header '{if(I$GSCNCATEGORY=="DEL" && I$GSCNQUAL>=1){print $0} \
        else if(I$GSCNCATEGORY=="DUP" && I$GSCNQUAL>=9){print $0} \
        else if(I$GSCNCATEGORY=="MIXED" && I$GSCNQUAL>=8){print $0} \
        else if($7=="MATCH_1KGP"){print $0} \
        else if($3~/^LUMPY/){print $0}}' \
    | bgzip -c \
    > gtex.lumpy.gs.1KGP_0.5.size.gscnqual.high_conf.vcf.gz
...
mv -f gtex.lumpy.gs.1KGP_0.5.size.gscnqual.af.high_conf.vcf.gz gtex.lumpy.gs.1KGP_0.5.size.gscnqual.high_conf.vcf.gz
tabix -p vcf -f gtex.lumpy.gs.1KGP_0.5.size.gscnqual.high_conf.vcf.gz
...
do
    bomb -g /ajscott/sv_aggregate -m 2 -J $SAMPLE.svcount -q hall-lab -o sv_count/logs/$SAMPLE.high.log -e sv_count/logs/$SAMPLE.high.log \
        "bash sv_counts.sh \
           gtex.lumpy.gs.1KGP_0.5.size.gscnqual.high_conf.vcf.gz $SAMPLE \
...
while read SAMPLE LONG BAM LUMPDIR
do
    bomb -g /ajscott/sv_aggregate -q hall-lab -m 1 -J $SAMPLE.svcount -e sv_count/logs/$SAMPLE.gs.high.log -o sv_count/logs/$SAMPLE.gs.qual_24.high.log "zcat gtex.lumpy.gs.1KGP_0.5.size.gscnqual.high_conf.vcf.gz | vawk '{if(I\$SVTYPE==\"CNV\"){if(S\$$SAMPLE\$CN != I\$MODECN){print I\$GSCNCATEGORY}}}' | sort | uniq -c > sv_count/$SAMPLE.gs.high.count.txt"
done < sample.map
...
# check that this adds up for GS variants. 37600+9829+5317 = 52746. check.
zcat gtex.lumpy.gs.1KGP_0.5.low_pass.vcf.gz | vawk '$3~"^GS"' | wc -l
# 37600



# check that it adds up for LUMPY variants. check.
zcat gtex.lumpy.gs.1KGP_0.5.low_pass.vcf.gz | vawk '$3~"^LUMPY" && !I$SECONDARY' | wc -l
# 74242
